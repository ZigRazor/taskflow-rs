...
impl Baseline {
    ...
    pub fn from_profile(profile: &ExecutionProfile, label: impl Into<String>) -> Self {
        ...
        let avg = if durations_us.is_empty() {
            0
        } else {
...
impl RegressionReport {
    pub fn summary(&self) -> String {
        if self.passed {
            format!(
                "OK No regressions detected (baseline: {})",
                self.baseline_label
            )
        } else {
            let crits = self
...
impl RegressionDetector {
    ...
    pub fn detect(&self, current: &ExecutionProfile) -> RegressionReport {
        ...
        let durations_us: Vec<u64> = current
            .task_stats
            .iter()
            .map(|t| t.duration.as_micros() as u64)
            .collect();
        ...
        let comparisons = vec![
            MetricComparison::new(
                "Total Duration (us)",
                bl.total_duration_us as f64,
                cur_total as f64,
                false,
            ),
            MetricComparison::new(
                "Avg Task Duration (us)",
                bl.avg_task_us as f64,
                avg as f64,
                false,
            ),
            MetricComparison::new(
                "P50 Task Duration (us)",
                bl.p50_us as f64,
                cur_p50 as f64,
                false,
            ),
            MetricComparison::new(
                "P95 Task Duration (us)",
                bl.p95_us as f64,
                cur_p95 as f64,
                false,
            ),
            MetricComparison::new(
                "P99 Task Duration (us)",
                bl.p99_us as f64,
                cur_p99 as f64,
                false,
            ),
...
mod tests {
    ...
    fn detects_total_duration_regression() {
        ...
        assert!(
            report
                .violations
                .iter()
                .any(|v| v.metric.contains("Total Duration")),
    ...
    fn improvement_does_not_fail() {
        ...
        let detector = RegressionDetector::new(baseline, RegressionThresholds::default());
        assert!(
            detector.detect(&im_profile).passed,
            "improvement should not flag violations"
        );
    }
    ...
    fn critical_severity_at_2x_threshold() {
        ...
        let v = report
            .violations
            .iter()
            .find(|v| v.metric.contains("Total"))
            .unwrap();
    ...
    fn strict_thresholds_flag_small_regressions() {
        ...
        assert!(
            d_default.detect(&rg_profile).passed,
            "should pass default thresholds"
        );

        let d_strict = RegressionDetector::new(bl_strict, RegressionThresholds::strict());
        assert!(
            !d_strict.detect(&rg_profile).passed,
