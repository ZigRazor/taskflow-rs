...
use taskflow_rs::{
    AffinityStrategy, Executor, HwTopology, HwlocWorkerAffinity, Taskflow, TopologyProvider,
};
...
fn demo_cache_hierarchy(topo: &TopologyProvider) {
    ...
    for (level, entries) in &by_level {
        ...
        for (i, cache) in entries.iter().enumerate() {
            println!(
                "     [{}] {} KB, line={} B, assoc={}, shared by {} CPUs",
                i,
                cache.size_kb,
                cache.line_size,
                cache.associativity,
            ...
            println!(
                "         CPUs: {:?}",
                &cache.shared_cpus[..cache.shared_cpus.len().min(4)]
            );
        }
    }
