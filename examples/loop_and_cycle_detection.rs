use taskflow_rs::{Executor, Taskflow, CycleDetector, Loop};
...
fn demo_loop_construct() {
    ...
    let c = counter.clone();
    let condition = taskflow.emplace(move || {
    ...
    // Create loop body
    let c = counter.clone();
    let body_task = taskflow.emplace(move || {
...
fn demo_complex_graph() {
    ...
    let result = detector.detect_cycle();
    println!("\n   Cycle detection: {}", if result.has_cycle() { "CYCLE FOUND" } else { "NO CYCLE" });
    ...
    let result = detector.detect_cycle();
    println!("   Cycle detection: {}", if result.has_cycle() { "CYCLE FOUND" } else { "NO CYCLE" });
