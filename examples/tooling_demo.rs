...
fn demo_profiling() {
    ...
    if let Some(profile) = profiler.get_profile() {
        ...
        println!(
            "   Longest task: {:?}",
            profile.longest_task().map(|t| &t.name)
        );
        println!(
            "   Shortest task: {:?}",
            profile.shortest_task().map(|t| &t.name)
        );
    }
...
fn demo_performance_monitoring() {
    ...
    println!(
        "   Worker 0 utilization: {:.2}%",
        metrics.worker_utilization(0)
    );
    println!(
        "   Worker 1 utilization: {:.2}%",
        metrics.worker_utilization(1)
    );
    println!(
        "   Average utilization: {:.2}%",
        metrics.average_worker_utilization()
    );
...
fn demo_visualization() {
    ...
    let dependencies = vec![(1, 2), (2, 3), (2, 4), (3, 5), (4, 5)];
