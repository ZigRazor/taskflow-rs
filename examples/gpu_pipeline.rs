...
fn run_image_pipeline(device: GpuDevice) {
    ...
    for image_id in 0..num_images {
        ...
        let preprocess = taskflow.emplace(move || {
            ...
            // Allocate GPU memory
            let mut input_buf =
                GpuBuffer::allocate(&dev2, data.len()).expect("Failed to allocate GPU memory");
            ...
            println!(
                "[Image {}] [GPU] Launch: {} blocks, {} threads/block",
                image_id,
        ...
        let extract = taskflow.emplace(move || {
            ...
            let mut feature_buf =
                GpuBuffer::allocate(&dev3, feature_size).expect("Failed to allocate features");
            ...
            feature_buf
                .copy_to_host(&mut features)
                .expect("Failed to copy features");

            let elapsed = start.elapsed();
            println!(
                "[Image {}] [GPU] Extracted {} features in {:?}",
        ...
        let classify = taskflow.emplace(move || {
            ...
            println!(
                "[Image {}] [CPU] Classified as: {}",
                image_id, classes[class]
            );
        });
    ...
    println!(
        "Average: {:.2} ms/image",
        total_elapsed.as_millis() as f64 / num_images as f64
    );
    println!("═══════════════════════════════════════");
}
